...
uint8_t char_value[4] = {0xff, 0xff, 0xff, 0xff};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    {
        ESP_LOGI(GATTS_TABLE_TAG, "GATT_READ_EVT, conn_id %d, trans_id %" PRIu32 ", handle %d", param->read.conn_id, param->read.trans_id, param->read.handle);
        for (int i = 0; i < sizeof(char_value); ++i)
        {
            char_value[i] = 0xff;
        }
        char_value[0] = ucFallProbPara;
        // esp_ble_gatts_send_indicate(gatts_if, param->read.conn_id, heart_rate_handle_table[IDX_CHAR_VAL_A],
        //                             sizeof(notify_data), notify_data, false);
        esp_ble_gatts_set_attr_value(param->read.conn_id, sizeof(char_value), char_value);
